...
def filter_accepted_rows(df, coverage, amino):
    ...
    for i, test_row in df.iterrows():
        ...
        if all(overlap < coverage for overlap in overlaps):
...
def ECODmapping(df):
    ...
    """
    Maps the 'query_name' column in the input DataFrame to the corresponding ECOD IDs
...
def dotate(hmm_file, mapping=False, hmm_cov_co=0.75, iE_score_co=0.01, domain_cov_co=0.75, unanotated_co=10, cores=1, chunksize=100):
    ...
    column_names = [
        ...
        'from_hmm', 'to_hmm', 'from_ali', 'to_ali', 'from_env', 'to_env', 'acc', 'description_of_target'
    ]

    # Load the HMM search results into a DataFrame
    df = pd.read_csv(hmm_file, delimiter=r'\s+', comment='#', header=None, names=column_names)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Unexpected error while storing data in SQL: {e}")
        exit(1)
